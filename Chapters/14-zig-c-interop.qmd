...
So using C code in Zig always involves performing these two steps above. However, when
...
- translating the C header file into Zig, through `zig translate-c`, and then, importing the translated Zig code.
...
Now, on a surface level, `@cImport()` versus `translate-c` look like
...
There is currently an accepted proposal at the Zig project, to move `@cImport()`
to the build system[^cimport-issue]. If this proposal is completed, then, the "use `@cImport()`"
strategy would be transformed into "call the translate C function in your build script".
...
your Zig module to start calling C code.
...
### You need to link your code with the C library

Regardless of which strategy you choose to use, you will always
have to link your Zig source code to the C library that contains the C code
