...
Now, how exactly the algorithm behind the base64 encoding works? Let me show you.
The base64 algorithm usually works on a window of 3 bytes. Because each byte have
8 bits, so, it forms a set of $8 \times 3 = 24$ bits. This is desirable because
24 bits is divisble by 6, which can form a set of 4 groups of 6 bits each.
...
Now you may think, what if I have a particular string that have a number of bytes
that is not divisible by 3? What happens? For example, if I have a string
...
becomes the string "SGk". Actually, if you use a real base64 encoder you will likely
get the string "SGk=" as output. I will tell you later about what this
final equal character (`=`). For now, let's ignore it.

![The logic of the base64 encoding](./../Figures/base64-flow.png){#fig-base64-algo1}

In the example of the string "Hi" we have 16 bits. So, we lack a full byte (8 bits)
to complete the full window of 24 bits that the algorithm work on. In essence,
everytime that the algorithm does not meet this requirement, it simply add extra zeros
until it fills the space it needs.
...
is to replace a runtime calculation (which can take a long computing time be done) by a basic array indexing
operation.

Instead of calculating the results everytime you need them, you calculate them once, and them, store the results in an array
(which behaves lake a "table"), and then, any time that you need to use this result, instead of
using many resources to calculate this result, you simply retrieve this result
from the array where you stored all the possible results. So we retrieve the result we need
...
to create a new instance of a `Base64` object, and, a `char_at()` function, which is a
...
In other words, the `char_at()` function is responsible for getting the character in the lookup table (i.e. the `table`) that
corresponds to a particular index in the "base64 scale". So, in the example below, we know that
the character that corresponds to the index 27 in the "base64 scale" is the character "c".
