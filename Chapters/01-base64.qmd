...
Let's use the first byte in the output of the base64 encoder as an example of what
...
So, don't take the "6-bit group" factor so seriously. We do not need necessarilly to
...
1. `output[0]` is produced by summing two components. First, move the bits from `input[0]` two positions to the left. Second, move the bits from `input[1]` 4 positions to the right. Then, sum these two components.
1. `output[1]` is produced by summing two components. First, move the bits from `input[1]` four positions to the left. Second, move the bits from `input[2]` 2 positions to the right. Then, sum these two components.
1. `output[2]` is produced by moving the bits from `input[2]` six positions to the left, then, you sum the result with `input[3]`.


The @tbl-6bit-decode presents how the above steps translate into Zig code:
