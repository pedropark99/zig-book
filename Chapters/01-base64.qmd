...
The algorithm behind a base64 decoder is essentially the inverse process of a base64 encoder, but it's a little
harder to explain, or to demonstrate visually how it works.
...
The base64 encoder worked on a window of 3 bytes, which creates 4 groups of 6 bits each. But,
a base64 decoder works on a window of 4 bytes. Because the decoder wants to convert these 4 bytes
back into the original sequence of 3 bytes from which they came from. So, once again, we are essentially reverting the process made
by the base64 encoder.

Each byte in the output string of the decoder (the original binary data) is normally created from
two different bytes in the input (the base64 encoded data).
In other words, each byte that comes out of a base64 decoder is created by transforming (or merging) two different
...
First, we take each base64 encoded character from the input, and we convert it back to its corresponding index in the base64 scale.
This is done by using the lookup table that represents our base64 scale. Then, we apply a series of bit transformations over these indexes
to produce the original bytes of the message.

The exact transformations, or, the exact steps applied over the base64 indexes to transform them
into the bytes of the output, are a bit tricky to visualize in a figure like this. Because of that, I have
summarized most of these transformations as "Some bit shifting and additions ..." in the figure. These transformations
will be described in depth later.

Besides that, if you look again in @fig-base64-algo2, you will notice that the character `=` was mapped to a NULL value
at the beginning of the algorithm. Remember, the character `=` does not belong to the base64 scale. It is just a special character
that marks the end of meaningful characters in the base64 sequence. Thus, if we try to map the character `=` to an index
in the base64 scale, we would get a NULL value (which corresponds to zero in binary), since it does not belong to the scale.
...
This binary sequence is the number 18 in decimal, and also, the value `0x12` in hexadecimal.
...
in each iteration of the for loop; 2) `output_index`, to hold the current index in the output;
3) `tmp_buffer`, which is the temporary buffer that holds the base64 indexes to be
...
indexes in `tmp_buffer` to be converted, and then, we apply the 6-bit transformation
