...
the algorithm does, is to check how to divide the input bytes into groups of 6 bits. This is the first
concern of the algorithm. It needs to know how the input bytes will be divided into groups of 6 bits.

If the algorithm notice that there is a group of 6 bits that have some bits in it, but is not full
(in other words, the number of bits in this group is greater than zero, but less than six), meaning that, it lacks
...
This group is necessary, because the algorithm works on 4 groups of 6 bits.
...
Therefore, everytime that the algorithm produces a "padding group", this group is mapped to `=`.
