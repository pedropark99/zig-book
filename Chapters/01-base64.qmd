...
The core part of the base64 encoder algorithm is the 6-bit transformation that was presented at @fig-base64-algo1.
...
If you have the perfect window of 3 bytes to work on, how the 6-bit transformation
is made in these bytes to produce the 4 bytes of output? The bulletpoints below
summarizes the process. I use the variable `output` to refer to the bytes in the output,
and the variable `input` to refer to the bytes in the input of the encoder.

- the `output[0]` is produced by moving the bits in the first byte in the input two positions to the right.
- the `output[1]` is produced by moving the bits in the byte `input[0]` four positions to the left.

Let's begin by building the first byte in the ouput of the encoder. This is the easiest byte of the 4 bytes in the output
to build, since we only need to extract the first 6-bits from the first and only byte of the input (`01001000`).
To collect these first 6-bits, we can use the *bit shift to the right* (`>>`) operator.
