...
built-in functions `@as()` or `@ptrCast()`. With these functions we can convert
...
We use the `@as()` function to explicit convert (or cast) a Zig value from a type `x`
...
But in this case, we are not converting any type of object.
More specifically, we are converting something into a pointer.
Everytime a pointer is involved in some "type casting operation" in Zig,
the `@ptrCast()` function is used. This `@ptrCast()` function is responsible
