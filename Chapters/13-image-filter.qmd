...
Now, before we continue, since we have opened the file with `fopen()`, is important
for you to remember to close the file at the end of the program, with `fclose()`.
...
<!--
Now, because we are going to use this image header data later in our program, to save the results
back into a new PNG file, w.
-->
...
other elements affecting this number, such as the color model used in the image, the bit depth, and others.

Anyway, all this means that calculating the size of the space we need is not simple, or,
straightforward. That is why the `libspng` library offers a utility function named
`spng_decoded_image_size()` to calculate this size.
...
In the snippet exposed above, you can see that I have created another small function named `calc_output_size()`, which encapsulates
the logic around this utility function, and returns a nice integer value as result, informing
the size of the space that we need to allocate.

You might quest yourself what the value `SPNG_FMT_RGBA8` means. This value is actually a macro defined in the `spng.h` header file.
This macro is used to identify a "PNG format". More precisely, it identifies a PNG file
that uses the RGBA color model and 8 bit depth. So by providing this macro as input to the `spng_decoded_image_size()` function,
we are saying to this function to calculate the size of the decoded pixel data considering a PNG file that follows this "PNG format".
...
Notice that this function is encapsulating the logic around the `spng_decode_image()`.
Also, we are using the `SPNG_FMT_RGBA8` macro once again to inform the corresponding function,
...
So, based on how these first 12 bytes (or these 3 pixels) look, we can say that is likely
...
Now, we can see that, in the first pixel in this image, we have 200 of red, 194 of green, and 216 of blue.
...
we can use the @eq-grayscale. You can read more about at the Wikipedia page for Grayscale [@wiki_grayscale].
...
Since we have now the grayscale version of our image represented in our buffer object,
we need to encode this buffer object back into a "PNG format", and save the encoded data into
