...
This might look as a subtle difference between them, but this makes all the difference.
In some cases, the `zig` compiler, or, more specifically, the embedded C compiler, might complain about the use of Zig objects
being given as input to C functions.

As an example we will take the code exposed below. Here we are using
the `fopen()` C function to open and close a file. Talking about how the `fopen()`
function works in C, it takes as input two C objects of type `*const char`, which is just a C string,
a.k.a. null-terminated array of bytes. But in this code example, we are passing the
string literals written in Zig directly to the `fopen()` function.
...
If we store the path to the `build.zig` file inside
a Zig object, like the `path` object below, and then, we pass this Zig object as input to the `fopen()` C function,
the program still compiles successfully. I have ommitted most of the code in this example below
just for brevitty reasons, since the remainder of the program is still the same.
The only difference between them is just these two lines exposed below.
...
However, if I force the `zig` compiler to interpret this `path` object as a Zig string object,
by annotating the `path` object with the data type `[]const u8`, then I actually get a compile error
as demonstrated below. Because now I'm explicitly using a Zig object into a C function.

So over the previous examples, the Zig compiler was somewhat automatically converting the string literals
that we were providing into C strings, and passing them to the C functions. But now that we have
declared our `path` object as a true Zig string object, we shall now properly convert it
into C strings if we want to use it as input to a C function.
...
This strategy works because this pointer to the underlying array found in `ptr` is identical
to a C pointer to an array of bytes, i.e. an object of type `*const char`.
This is why it solves the problem that we had before.

Another option to solve this problem, is to convert the Zig string object into a C pointer by using the
built-in functions `@as()` and `@ptrCast()`. With these functions we can convert
