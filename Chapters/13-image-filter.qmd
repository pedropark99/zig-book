...
In some cases, you may have troubles when passing Zig objects directly as inputs to a C function.
In this section I want to give you some more context about this. In summary, what you will discover
in this section is that the `zig` compiler is capable of safely converting Zig objects
into C compatible values in some specific cases.

Most of these specific cases happen when you do not give an explicit Zig type to this Zig value.
This is when you let the `zig` compiler to infer which is the appropriate data type to use in that situation.
In constrast, when you do give an explicit Zig type to a Zig value, then, the compiler is forced to interpret
...
You become responsible for converting
