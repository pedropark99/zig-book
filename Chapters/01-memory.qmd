...
Lengths that grow during runtime are intrinsically associated to "runtime known" type of values.
In other words, if you have a variable whose length might grow during runtime, then, the length
of this variable becomes not known at compile time. If the length is not known at compile-time,
the value of this variable also becomes not known at compile-time.
These types of variables are usually stored in the heap memory space, which is
a dynamic memory, which can grow or shrink to fit the size of your variables.
...
The `r` variable (which is declared outside of the `add()` function scope) is also stored in the stack.
But since it is declared in the "outer" scope, this variable is stored in the
...
also counts for any variable declared inside the scope of your `main()` function for example.
...
Using the `add()` function that we exposed above as an example, it means that the variable `result` is automatically
destroyed once the function returns.

::: {.callout-important}
Local variables that are stored in the stack space of a function are automatically
...
variable in the scope of a for loop, this local variable is accessible only within the scope
...
While the stack accepts only variables whose length is known at compile-time, the heap is a much more dynamic
...
1. every variable (constant or not) whose value **is not known at compile time** is, at first glance, stored in the stack space for the current scope.
...
Also, there is another where you might want to use an allocator, which is when you want to write a function that returns a pointer
to a local variable. As I described at @sec-stack, you cannot do that if this local variable is stored in the
stack. However, if this variable is stored at the heap, then, you can return a pointer to this variable at the
...
a memory type that can grow together with your variables, or that you
...
of task. While, the `c_allocator()` is a alias to the C standard allocator, which is `malloc()`. So, yes, you can use
`malloc()` in Zig if you want to. Just use `c_allocator()` from the Zig standard library. However,
if you do use `c_allocator()`, you must link to libc when you compile your source code with the
`zig` compiler.
...
instead of just a small piece of it. The size of this page depends on the system you are using.
...
work with a fixed size buffer that is stored in the stack. So these two allocators only allocates
...
to allocate more memory many times you want, but, at the same time, you only need to free all the memory you allocated once.
...
was allocated in the variable `input` is capable of storing a string that is
50-characters long.
