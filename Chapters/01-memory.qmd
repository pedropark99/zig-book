...
the `zig` compiler can figure it out what is the exact value of a particular object (or object)
that exists in your source code.

For example, in the source code below, we have two constant objects (`name` and `array`) declared.
...
stack space. Also, every local object that you declare inside the function scope are
...
While the stack accepts only objects whose length is known at compile-time, the heap is a much more dynamic
...
1. every object (constant or not) whose value **is not known at compile time** is, at first glance, stored in the stack space for the current scope.
...
1. the heap can only accessed through allocators. If your object were not created through the `alloc()` or `create()` methods of an allocator object, then, it he is most certainly not an object stored in the heap.
