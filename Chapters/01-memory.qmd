...
as input to the function. In the example below, I am using the `DebugAllocator()` as my allocator
...
The `DebugAllocator()`, as the name suggests, is a "general purpose" allocator. You can use it for every type
...
If you give, for example, a `DebugAllocator()` object as input to the `ArenaAllocator()` constructor, like in the example below, then, the allocations
you perform with `alloc()` will actually be made with the underlying object `DebugAllocator()` that was passed.
