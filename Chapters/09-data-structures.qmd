...
interfaces [@geeks_generics]. In other words, a generic is a class (or method) that can work
with multiple data types.

For example, in Java, generics are created through the operator `<>`. With this operator,
a Java class is capable of taking a data type as input, and therefore, the class can fit
...
Now that we have seen an example of a generic function, let's take a look at a generic struct.
Generic structs are the Zig way to replicate a generic class from Java,
or, a class template from C++.

To create a generic struct in Zig, we normally use a function that creates this generic struct object.
In other words, this function behaves as a "constructor" for the generic struct object.
...
Just as a first example, look at this 
