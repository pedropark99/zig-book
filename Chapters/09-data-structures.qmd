...
Some professionals know this type of data structure by different terms, like "map" or "associative arrays". But most professionals
know this structure by *hashtable*.
The Zig Standard Library provides different implementations of a hashtable data structure
at the module `std.hash_map`. Each implementation have it's own cons and pros, which we will
discuss later on.
...
A hashtable is a data structure that stores values by using a key. In other words, it is
a structure based on key-value pairs. In more details, a hashtable use a hash function
to compute an index. This index is actually an array index, it points to an
element in an array. Inside this array element you find the stored value.

The struct `HashMap` from this module provides a general-purpose hashtable,
which have very fast operations (lookup, insertion, deletion), and also,
quite high load factors for low memory usage.
