...
A `HashMap` object also offers a set of "fetch operations", a fetch operation
returns as output the key-value pair that you are performing the action over. These fetch operations
are performed by the methods `fetchPut()` and `fetchRemove()`. As their name suggests, they
are used to insert and to delete values from the hashtable.

But when you use these methods, they not only insert or delete the value, but they also return
as output a key-value pair containing the key and the value that were used in this operation.
