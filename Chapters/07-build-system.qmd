...
FreeType is one of the most widely used pieces of software.
...
Since FreeType is a C library, I link it against `libc` through the `linkLibC()` method.
Just as an example, I will build FreeType as a static library. That is why I use
the `addStaticLibrary()` method below to create the target object to build the library.
...
You can list the C compiler flags to be used in the compilation process
...
In theory, there is nothing stopping you from using this array to also add include paths (with the `-I` flag),
to the compilation process. But there is a formal way in Zig to include these paths, which is
discussed at @sec-include-paths.

In Zig, we add C flags to the build process
together with the C files that we want to compile, using the
`addCSourceFile()` and `addCSourceFiles()` methods.
So in the example above, we have just declared the C flags that we want to use.
But we haven't added them to the build process yet.
To do that, we first need to list the C files to be compiled.

### Listing your C files

The C files that contains "cross-platform" code are listed in the `c_source_files`
object below. These are the C files that are included by default in every platform supported by the library.
Since the amount of C files in the FreeType library is big, I omitted
the rest of the files in the code example below, for brevity purposes.
...
Now, there are some C files that are platform-specific, meaning that, they are only
included in the build process on specific target platforms. The objects that list
these C files are exposed in the code example below.
...
Both of these functions are methods from a `Compile` object (i.e. target object).
...
across all files, you will probably find `addCSourceFiles()` a better choice.
...
are using the `os.tag` that we learned at @sec-detect-os, to add the platform-specific
C files.
...
C Macros are commonly defined through the `-D` flag
from a C compiler.
...
Every header file that you include in your C source code, through a `#include` statements needs to
