...
When you declare a new build step in your build script, this step
...
unit tests in the project at once.
...
script with the `build` command from the `zig` compiler. After the build
process is finished, we can simply execute this `unit_tests` executable
in the terminal.

Though, if you remember from the previous section, we already learned
...
Now that we registered this new build step, we can call it
and trigger the execution process by calling the command below
