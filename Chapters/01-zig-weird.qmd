...
So, if you have experience with Python, or Javascript, for example, it wiil be fine.
...
are what makes C++ so complex <!-- or: so much more complex -->and hard to learn. Zig tries to go in the opposite direction.
...
C are an evil
<!-- i wouldn't use that word in a programming book. what about "frequent"? -->
source of confusion. They really make it sometimes hard to debug
...
of the code are being sent to the compiler.
<!-- grammar: 'it' is singular, 'macros' is plural. To what refers 'it' is not clear here.
i believe you meant to say 'macros can obscure the code you wrote'? -->
It obscures the actual source code that you wrote.

You don't have macros in Zig. In Zig, the code you write, is the actual code that get's compiled by the compiler.
You don't have evil
<!-- evil again :-) just drop it, it doesn't add any information to the text -->
features that obscures you code.
...
<!-- you shouldn't use generalization, try this:
Low-level languages use a compiler to build your
...
<!-- you explain the same fact twice because you begin next sentence with
'in other words'. Why can't you make your point in the first sentence. The entire paragraph
is confusing. -->
...
For example, you do have to explicitly specify the type of every 
<!-- 'each function argument' instead? -->
single function argument, and also,
...
<!-- confusing sentence: build for poeple to use so they can use it. -->
If you have a project where you are building
a library for other people to use then you need to expose your functions
...
The keyword `export` is a keyword used in libraries written in Zig.
