...
Zig is a language more closely related to C (which is a procedural language), than C++ or Java. Because of that, you do not
have advanced OOP (Object-Oriented Programming) patterns available in Zig, such as classes, interfaces or
class inheritance. OOP in Zig is possible by using struct definitions, like the same way you would do in C.
...
You declare the data members of this new struct you are creating, and,
also, you can register functions as methods that belongs to this particular struct. So that every object,
that you create with this new type, have the same methods associated with them.

In C++, we normally have a constructor method (or, a constructor function) to construct or to instantiate every object
of the particular class you are using.
And you also have a destructor method (or a destructor function) that
is responsible for destroying every object of that class.

In Zig, we normally declare the constructor and the destructor methods
of our structs, by declaring an `init()` and `deinit()` methods. This is just
a naming pattern that you will find across the entire Zig standard library.
So, the `init()` method of a struct normally represents the constructormethod of that class, while the `deinit()` method is the method used for destroying an existing instance of that struct.
