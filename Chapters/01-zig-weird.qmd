...
Examples of build systems are GNU Make, GNU Autoconf and Ninja[^cmake],
...
[^cmake]: C++ programmers also like to use CMake to build their projects, but CMake is not exactly a build system (and that is the only reason why it's not mentioned here in this list). Technically, CMake is a build system generator. It generates build scripts for other build systems, such as Ninja or Make.
