...
We already saw both the `init()` and `deinit()` methods being used at @sec-arena-allocator. In this section,
...
As an example, let's build a simple `User` struct to represent an user of some sort of system.
...
Also, notice in this example, that we registrated an `init()` function as a method
...
In essente, the `zig` compiler will look for some hint of what is the type of that struct.
...
Anonymous structs are very commom to use on function arguments.
...
While the second argument is a struct literal that contains the values
