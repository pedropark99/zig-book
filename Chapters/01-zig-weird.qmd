...
Zig is a very young language which is actively developed.
...
You should have some experience with script languages like Python or Javascript.
Better if you have experience with a low-level language such as C, C++ or Rust.

## What is Zig?

Zig is a modern, low-level, and general-purpose programming language. Some programmers think of
Zig as modern better version of C.

The author thinks Zig follows the principle of "less is more".
Instead of trying to become a modern language by adding more and more features,

<!-- do you really want to *remove* features from C?? -->
Zig attempts to simplify and modernize by having more consistency and robustness.
Its goal is to make writing, reasoning and debugging easier and simpler.

This philosophy becomes clear with the following phrase from the official website of Zig:

> "Focus on debugging your application rather than debugging your programming language knowledge".

<!-- I thought this would be about Zig and not about C++?? -->
Zig is a very simple language closely related to other languages such as C and Go.

<!-- again ranting about C, why would you do that in Zig text? -->
Better consistency means all coding tasks are performed in the Zig language. There are no pre processor
statements necessary like in C.
The Zig code you write is the actual code that get's compiled.
Better transparency because no functions or operators from the standard library allocate memory.

Zig becomes clearer and easier to read and to write.
It achieves a robust state with consistent behaviour in edge situations.

Less is more.

## Hello world in Zig

Since the first C book by Kerninghan&Ritchie it became tradition to begin the programming journey
with the famous "Hello World" program.

It's easy to start a new project. Navigate to a directory of your choice and execute the following commands.
...
A "source" (`src`) directory with the two text files `main.zig` and `root.zig`. Each file represents a module.
...
The `ìnit` command creates two additional files in the work directory - `build.zig` and `build.zig.zon`.
`build.zig` represents the build script for your project and it is written in Zig (consistency!).
It will be executed with the command `zig build`.
You won't need an external build system when using Zig. The file `build.zig` brings all the code necessary to build the entire project.

<!-- why explaining details of other langugages? This book is about Zig, not a general introduction into programming -->

Instead of using a separate build system we use the
Zig language itself to write our build scripts.
Zig brings its own native build system.[^zig-build-system].

[^zig-build-system]: <https://ziglang.org/learn/overview/#zig-build-system>.

The file `build.zig.zon` is the configuration file for the Zig package manager.[^similar to Python's pip, Rust's cargo, Javascript's npm or Ruby's gem]

### The file `root.zig` {#sec-root-file}

Let's look into the `root.zig` file.
Mou might notice that every line of code with an expression ends with a semicolon (`;`). This follows
the syntax of C-family programming languages.[^https://en.wikipedia.org/wiki/List_of_C-family_programming_languages]

Notice the `@import()` call at the first line. 
This built-in function imports functionality from other Zig modules into the current module.
Importing the `std` module makes the functions from the Zig standard visible.

Zig's assignment operator is `=`.
A new objecty is created by assigning a value to a name 

```{zig}
(const|var) name = value;
```
In the example below, we are creating two constant
...
In the example we are declaring a function called `add()`, which has two arguments named `a` and `b`.
The function returns an integer of the type `i32` as result.

Zig is not a strongly-typed language, because you can omit
the type of an object if it can be derived from the assigned value.

Remember, you do have to specify the type of each function argument and the return value.

We specify the type of an object or a function argument in Zig, by
using a colon character (`:`) followed by the type after the name of the object.
The expressions `a: i32` and `b: i32` define both with the type `i32`,
which is a signed 32 bit integer.

The return type of the function follows function arguments list.

If the function is preceded by `export` keyword then it exposes the function can be called from outside the library.
