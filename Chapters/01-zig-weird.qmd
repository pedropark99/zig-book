...
In contrast, if you use `var`, then, the object created is a variable (or "mutable") object.
...
Also, notice in the example below that we have explicitly annotated the data type
of the `age` object with the `u8` type. The basic idea is, when we use a variable/mutable object,
the Zig compiler ask for us to be more explicit with what we want, to be more clear
about what our code does.
