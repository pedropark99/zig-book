...
In my (author's) personal interpretation, Zig is tightly connected with "less is more".
...
As a result, analyzing, writing and debugging Zig applications are much easier and simpler than in C or C++.
...
don't have functions or operators from the standard library that makes
hidden allocations behind your back.
...
```bash
zig init hello_world
```


First, notice that we have a `main()` function in this example. As you would expect
from a C, C++, Rust and Go program, we also have a `main()` function in Zig.

If you are building a library in Zig, then, you most likely do not need to declare a `main()`
function in your code. However, if you are building an executable program, then,
you must declare a `main()` function. Because without this main function, you
get no entrypoint to your executable. How you are going to execute your program
without an entrypoint?

Secode, I want to emphasize the line ends. Notice that every line of code
that have an expression, ends with a semicolon character (`;`). This is
similar to other languages such as C, C++ and Rust,
...
## Identifiers and creating new objects in Zig

In this section, I want to teach you about how you create new objects (or, new identifiers) in Zig.
...
On the other side, if you use `var`, then, you are creating a variable object.
So, you can change the value of this object as many times you want.

### Constant objects

For example, in the code example below, we are creating a new constant object called `age`.
...
### There is no such thing as unused constant objects

Every constant that you declare in Zig **must be used in some way**. You can give this constant
to a function call, as a function argument, or, you can use it in another expression
to calculate the value of another constant, or, you can call a method that belongs to this
particular constant.

It doesn't matter in which way you use it. As long as you use it.
If you try to break this rule, i.e. if your try to declare a constant, but not use it,
the `zig` compiler will not compile your Zig source code, and it will issue a error
message warning that you have unused constants in your code.

Let's demonstrate this with an example. In the source code below, we declare a constant
...
Everytime you declare a new constant in Zig, you have two choices:

1. you either use the value of this constant;
2. or you explicitly discard the value of the object;

To explicitly discard the value of any object (any object, constant or variable, whatever), all you need to do is to assign
...
This same rule applies to variable objects. If you assign a variable object to the underscore,
...
### You must mutate your variable objects

Every variable object you create in your source code must be mutated, at some point
in your souce code. In other words, if you declare an object as a variable
object, with the keyword `var`, if you do not change the value of this object
at some point in the future, the `zig` compiler will detect this error,
