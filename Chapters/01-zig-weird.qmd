...
Examples of build systems are GNU Make, GNU Autoconf and Ninja[^cmake1],
...
[^cmake1]: C++ programmers also like to use CMake to build their projects, but CMake is not exactly a build system (and that is the only reason why it's not mentioned here in this list). Technically, CMake is a build system generator. It generates build scripts for other build systems, such as Ninja or Make.
...
- explicitly handle this error inside the function body.
...
One very common activity is to select specific portions of an array you have in your source code.
In Zig, you can select a specific element from your array, by simply providing the index of this particular
element inside brackets after the object name. In the example below, I am selecting the third element from the
`ns` array. Notice that Zig is a "zero-index" based language,
like C, C++, Rust, Python, and many other languages.

Also notice in the example below that I am using the `stdout.print()` function to print the value of this element to the terminal.
We have seen this function before in the `main.zig` file at @sec-main-file. You will see this `stdout` object being mentioned constantly
across this book. This object gives us access to the OS stdout channel, through which you can print/show output to the user of
your program. We will talk more about it at @sec-filesystem.

For brevity reasons, I will constantly omit the definition of this `stdout` object in the code examples, just to not repeat myself
and occupy unnecessary space in this book. For now, just remember that you can always go back to the @sec-main-file section
to see the complete definition of this `stdout` object.
