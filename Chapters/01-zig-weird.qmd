...
language, not necessarily a low-level one. If you have experience with
 Python or JavaScript for example, you will be fine. If you have experience
...
table are actually about removing annoying behaviors and features from C and C++.
...
and at the same time achieves a much more robust state, with more consistent
...
are created in your current directory. First, a "source" (`src`) directory
...
`build-lib`, and `build-obj`, into the terminal,
...
If you try to break this rule, i.e., if you try to declare an object, but not use it,
the `zig` compiler will not compile your Zig source code, and it will issue an error
...
- Floating-point numbers: `f16`, 16-bit floating-point; `f32`, 32-bit floating-point; `f64`, 64-bit floating-point; `f128`, 128-bit floating-point;
...
and runtime known throughout this book, especially in @sec-compile-time.
...
to calculate the size of a file at compile-time, but these are rarely practical in a real application.
...
In summary, a sentinel-terminated array is just a normal array, but it
contains a "sentinel value" as the last element of the array. With a sentinel-terminated array,
...
NUL byte, that is, a byte with value `0x00`, which represents **nothing**, unlike characters such as `'M'` (`0x4D`) or control characters like linefeed `'\n'` (`0x0A`).
...
You can also see that this is a NUL-terminated array because of the zero after the `:` character in the data type.
