...
is basically making the `add()` function a "public function from this module", which
essentially means that this function is exposed for external modules to use it.
We talk more about this at @sec-pub-keyword.
...
done in Zig by using the expression `std.fs.File.stdout()`. If you use this expression to instantiate
...
fn read_file(path: []const u8, buffer: []u8) !usize {
    ...
    const nbytes = try file.read(buffer[0..]);
    return nbytes;
}
