...
An error union is a special type of an union object.
...
We can see that in the `ReadError` error union that we showed earlier in the `fill()` function,
...
it is an error union defined in the `http.Client` module, and therefore, it actually represents
...
different error values into a single object, and then, we use this object into the return type of the functions.
...
So, an error union is just a convenient way of grouping a set of
