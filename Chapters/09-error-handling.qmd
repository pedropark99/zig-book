...
In Zig, you can take the error values produced in your code, and pass them as inputs to functions.
You can also cast them into a different type of error value.

This have some similarities with exceptions in Python. Because in Python, when an exception happens
inside a `try` block, you can use an `except` block to access this exception value,
