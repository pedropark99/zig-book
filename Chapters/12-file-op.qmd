...
read operations performed in an unbuferred IO environment versus a buffered environemnt.
Let's suppose that we have a text file that contains the famous Lorem ipsum text[^lorem]. Let's also suppose that these diagrams at @fig-buff-diff
are showing the read operations that we are performing to read the Lorem ipsum text from this text file present in our
filesystem.

The first thing you notice, is that in an unbuffered environment the read operations lead to many system calls.
...
When we use a buffered IO system, when we perform the first read operation, instead of sending one single byte directly
to our program, the OS first send a chunk of bytes from the file at once to a underlying buffer object (i.e. an array object).
...
this next byte is already cached and ready to go.
...
![Buffered IO](./../Figures/buffered-io2.png){#fig-buffered-io}
...
of the file and caches it into the buffer object. As long as your program does not consume all of the 4096 bytes,
not a single system call is created.

A new system call is made only when you consume all of the 4096 bytes from the buffer. In this situation,
the new system call made essentially asks the OS to send the next 4096 bytes in the file, and once again,
they are cached into the buffer object, and the cycle starts once again.
...
that we presented at @sec-writer-reader are not buffered, which means that these objects
