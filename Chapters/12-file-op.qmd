...
to read, so, it is not garanteed that you will get precisely $n$ bytes as result.
...
Like it's brother (`getStdOut()`), this function also returns a file descriptor object that describes the `stdin` channel
of your OS.
...
consequence, achieve a much higher performance. At @fig-buff-diff you can find two different diagrams which presents the difference between
read operations performed in an unbuferred IO environment versus a buffered IO environment.
