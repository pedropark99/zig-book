...
If you execute this program, you will notice that it stops the execution, and starts to wait indefinitely
...
So, when we flush our IO buffer, we are effectively committing the chunk of data that is present in the IO buffer to
...
This will make sure that the bytes/data that you are writing are effectively written into the IO resource described by your file descriptor
object.
...
on the file. In Zig, this is no different. So every time we create a new file, this file remains
...
`fopen()` C function[^c-open]. If you open a file with a mode that does not allow the specific operation
that you want to perform on this file, you normally get a `AccessDenied` error from the OS as a result.

Now, for Windows users is worth noting that, most of the times, when you try to write/append data to a file,
you will most likely to use a `read_write` mode instead of a `write_only` mode. This happens because the Windows OS
also need a read permission over a file to be able to write data to it. Be careful with this. On Linux-like systems this usually does not happen,
and therefore, a `write_only` mode is usually sufficient if you just want to write data to a file, and nothing else.

In the code example below, we are opening the `foo.txt` text file with a `read_write` mode[^windows-write],
...
[^windows-write]: This makes this code example compatible not only with Linux, but also, with Windows, because of the reasons that I just explained in the previous paragraph. If you are a Linux-only user, you could use a `write_only` mode instead of a `read_write` mode, and the code would still work.
