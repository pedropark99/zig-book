...
Mutexes are a type of lock suited for objects and regions of memory. However, sometimes, we also need
to use other types of locks, specially for things that are not inside memory. For example, files in the filesystem
are usually stored in the hard disk of your computer, instead of in the memory.

Files in the filesystem might also be the source of race condition problems in your program. In other words,
you might cause different types of race conditions in your program, if two or more threads try to read from or write to
the same file at the same time.

The Zig Standard Library supports read/write locks through the `std.Thread.RwLock` module.
The locks implemented in this Zig module are semantically equal to the the
read/write locks from the `pthreads` C library, which corresponds to the `pthread_rwlock_t` C struct.
