...
Mutexes are a classic component of every thread library. In essence, mutexes are a type of "lock", and they are related to thread syncronization.
...
The main idea behind a mutex is to help us to control the access to data, and to
prevent two or more threads to read from or write to the same data at the same time.
...
When you have an object that is shared will all threads, and, you want to avoid two or more threads from
accessing this same object at the same time, then, you want to use a mutex to lock this specific object.
...
unlocked to continue. In other words, the execution of the thread is paused while the object
...
at the same time. If you change this code to add some microseconds of sleep between the first and the second calls to `spawn()`,
we will increase the chances of the program producing the "correct result".
...
three basic operations inside the for loop, which are:
...
Notice that, in the data race scenario, the read performed by a thread B happens
