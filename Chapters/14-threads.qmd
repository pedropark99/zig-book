...
Thread pools is a very popular programming pattern, which is used specially on servers and daemons. A thread pool is just a
...
this thread receives the next task registered in this "queue of tasks", then,
...
configurations for the pool of threads. The most important configuration in this struct object is
the members `n_jobs` and `allocator`. As the name suggests, the member `allocator` is an allocator object,
...
`Thread` object. The method have the same arguments as the previous one, but instead of creating a new thread, this `spawn()` method from
...
and, therefore, major bugs and undefined behaviour that are usually difficult to understand.
...
and, therefore, the other people (in this case, the other threads) that wants to use the bathroom at the same time
...
When a thread tries to access an object that is locked, this thread patiently waits for the object to be
unlocked to continue it's execution. In other words, the execution of the thread is paused while the object
...
(i.e. an object, a file descriptor, something that all threads have access to).
When we use mutexes, we want to lock the shared resource at the beginning of the critical section,
...
They summarise well the role that a critical section plays in the thread syncronization problem [@geeks_critical_section].
...
to solve, which can be summarized into race conditions.
