...
performed inside this context, by providing the name of a function in the second argument of the `spawn()` method.
...
In other words, everytime you call `spawn()`, not only a new thread gets created,
but also, the "start work button" of this thread gets automatically pressed.
...
which also starts the execution at the moment that the thread gets created.


## Returning from a thread

We have learned in the previous section that the execution of the thread starts at the moment
that the thread gets created. Now, we will learn how to "join" or "detach" a thread in Zig.
...
a different answer in each time that a different person gets to the target location first than the others.
...
the scenario exposed in @tbl-data-race-ideal do not suffer from a data race, because
...
But canceling a thread like this is bad. It's dangerously bad. As consequence, the Zig implementation
