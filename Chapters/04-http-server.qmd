...
Then, I call the `peekArray()` method of this `reader` object, together with `@memcpy()`,
to effectively read the first 400 bytes of the request, and save this data sent
by the client into the buffer object that we've created earlier.
...
Since we are only reading the first 400 bytes of the HTTP request in our code,
