...
first. At first, this stack data
structuree will store only `u32` values. But then, we are going to make it "generic".
...
[^cmemset]: <https://www.tutorialspoint.com/c_standard_library/c_function_memset.htm>
...
So how can we add a new value to the `Stack` object that we have? The
`push()` function exposed below is an answer to this question.
This function is shlightly complex, so let's describe it, step by step.

First, we have an if statement in this function. This if statement is
checking whether we need or not to expand the underlying array to store
...
to assign our new and bigger array to `self.items`. This is the end of the sequence
of necessary steps to expand our array.
...
value of the `length` attribute by one.



### Implementing the `pop` operation

Now, we need to implement the pop operation of our stack object.
This is the easiest operation, and the pop method below summarises
all the logic that is needed.

We just set the top element in the stack to zero, to indicate that
this element is "empty", and, after that, we also need to decrease
...
If we take only the section of our `Stack` struct with the `init()` method,
the function below represents a generic function that creates a generic
stack data structure.


```{zig}
#| eval: false
fn Stack(comptime T: type, capacity: u32) type {
    return struct {
        items: []T,
        capacity: u32,
        length: u32,
...
The full source code discussed in this chapter is freely available at the official
repository for this book. Just checkout the [`stack.zig` module](https://github.com/pedropark99/zig-book/tree/main/ZigExamples/data-structures/stack.zig)[^zig-stack]
